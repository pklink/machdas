<template>
    <div class="sixteen wide column">

        <h5 class="ui top attached header">Tasks</h5>
        <div class="ui bottom attached segment">
            <div class="ui relaxed bulleted list">
                <div class="item">Set the priority of a task by typing <code>@high</code>, <code>@normal</code> or <code>@low</code></div>
                <div class="item">The default priority is <code>@normal</code></div>
                <div class="item">Mark a task as completed by typing <code>@done</code></div>
            </div>
        </div>

        <h5 class="ui top attached header">Shortcuts</h5>
        <div class="ui bottom attached segment">
            <div class="ui relaxed bulleted list">
                <div class="item">Press <kbd>N</kbd> to focus the input field for creating a new task</div>
                <div class="item">Press <kbd>C</kbd> to create a new card</div>
                <div class="item">Press <kbd>H</kbd> to show this help page</div>
                <div class="item">Press <kbd>ESC</kbd> in an input field to cancel whatever you do</div>
                <div class="item">Press <kbd>Enter</kbd> in an input field to save it.</div>
            </div>
        </div>

    </div>
</template>